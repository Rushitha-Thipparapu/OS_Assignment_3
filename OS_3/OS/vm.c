...
//==
// Reserve virtual pages from oldsz..newsz but DO NOT allocate physical pages.
// This creates inner page table pages (walkpgdir(...,1)) but leaves each PTE == 0.
int allocuvm_lazy(pde_t *pgdir, uint oldsz, uint newsz)
{
    ...
    a = align_up(oldsz, PTE_SZ);
    for (; a < newsz; a += PTE_SZ) {
        // allocate/ensure inner page table exists but DO NOT back the PTE with physical memory
        pte = walkpgdir(pgdir, (void*)a, 1);
        if (pte == 0) {
            cprintf("allocuvm_lazy: walkpgdir failed\n");
            deallocuvm(pgdir, newsz, oldsz);
            return 0;
        }
        if ((*pte & PE_TYPES) != 0) {
            // If mapping already present, this is unexpected for reservation
            panic("allocuvm_lazy: remap existing mapping");
        }
        // leave *pte == 0 (non-present) so fault will trigger upon first access
    }
    return newsz;
}

// Allocate a physical page and install it for the given faulting virtual address.
// Returns 0 on success, -1 on failure (invalid addr or out of memory).
int
allocpage_on_fault(uint fault_va)
{
    char *mem;
    pte_t *pte;
    uint va = fault_va & ~(PTE_SZ - 1);   // Align down to page boundary

    // Debug: Print what fault we are handling
    cprintf("allocpage_on_fault: handling fault_va=0x%x proc=%d proc->sz=0x%x\n",
            fault_va, proc ? proc->pid : -1, proc ? proc->sz : 0);

    // Only valid for a user process and within its allocated size
    if (proc == 0 || va >= proc->sz) {
        cprintf("allocpage_on_fault: invalid fault (va=0x%x >= sz=0x%x)\n",
                va, proc ? proc->sz : 0);
        return -1;
    }

    // Get PTE slot; allocate inner page table if needed
    pte = walkpgdir(proc->pgdir, (void*)va, 1);
    if (pte == 0) {
        cprintf("allocpage_on_fault: walkpgdir failed for va=0x%x\n", va);
        return -1;
    }

    // If already mapped, nothing to do
    if ((*pte & PE_TYPES) != 0) {
        cprintf("allocpage_on_fault: page already mapped for va=0x%x\n", va);
        return 0;
    }
    ...
    mem = alloc_page();
    if (mem == 0) {
        cprintf("allocpage_on_fault: alloc_page failed\n");
        return -1;
    }
    ...
    // Install PTE: physical addr + AP (user read/write) + cache/buffer/type flags
    *pte = v2p(mem) | ((AP_KU & 0x3) << 4) | PE_CACHE | PE_BUF | PTE_TYPE;

    // Flush entire TLB (existing helper)
    flush_tlb();

    // EXTRA: ensure CPU is using the updated page table (TTBR0)
    // This makes the new mapping visible to the processor before retrying the instruction.
    switchuvm(proc);

    // Debug print
    cprintf("allocpage_on_fault: pid=%d va=0x%x -> pa=0x%x\n",
            proc->pid, va, PTE_ADDR(*pte));
...
pde_t* copyuvm (pde_t *pgdir, uint sz)
{
    ...
    // copyuvm: handle present pages and reserved (non-present) PTEs
    for (i = 0; i < sz; i += PTE_SZ) {
    	pte = walkpgdir(pgdir, (void *) i, 0);
   	if (pte == 0) {
       	    // parent has no inner page table here: create one for child and continue
            if (walkpgdir(d, (void *) i, 1) == 0) goto bad;
       	    continue;
    	}
    	if (!(*pte & PE_TYPES)) {
            // parent reserved this virtual page (non-present). Create a slot for child
            if (walkpgdir(d, (void *) i, 1) == 0) goto bad;
            continue;
    	}
    	// page is present in parent: copy it for child
    	pa = PTE_ADDR (*pte);
    	ap = PTE_AP (*pte);
    	if ((mem = alloc_page()) == 0) {
            goto bad;
...
kvminfo(void)
{
  extern pde_t *kpgdir;   // kernel page directory
  cprintf("Kernel page table dump:\n");

  // Kernel mappings use 1 MB sections
  for(uint va = 0; va < 0x80000000; va += PDE_SZ){  // up to KERNBASE
    pde_t pde = kpgdir[PDE_IDX(va)];
    if((pde & PE_TYPES) == KPDE_TYPE){
      cprintf("VA 0x%x -> PA 0x%x | type=section | AP=%d\n",
              va, align_dn(pde, PDE_SZ), pde & 0x03);
    }
  }
}

// Wrapper to expose PTE lookup for sys_pgpte
pte_t*
