...
int growproc(int n)
{
    ...
    if(n > 0){
        // Use lazy allocation version (no physical pages allocated yet)
        if((sz = allocuvm_lazy(proc->pgdir, sz, newsz)) == 0)
            return -1;
    } else if(n < 0){
    ...
    // DEBUG: print new size after grow
    cprintf("growproc: pid=%d new sz=0x%x\n", proc->pid, proc->sz);
