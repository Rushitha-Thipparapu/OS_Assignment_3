...
#include "proc.h"
//--
extern int allocpage_on_fault(uint);
//--
// trap routine
...
#define PTE_SZ 4096   // 4 KB page size (if not already defined)
...
void dabort_handler(struct trapframe *r)
{
    ...
    // Disable interrupts while handling the abort
    cli();

    // Save trapframe for current process so returning to user uses correct context
    if (proc != NULL) {
        proc->tf = r;
    }

    // Read Data Fault Status Register (DFS) and Fault Address Register (FAR)
    asm volatile("MRC p15, 0, %0, c5, c0, 0" : "=r" (dfs));
    asm volatile("MRC p15, 0, %0, c6, c0, 0" : "=r" (fault_addr));

    // Align faulting address to page boundary
    uint va = fault_addr & ~(PTE_SZ - 1);

        if (proc && va < proc->sz) {
        if (allocpage_on_fault(va) == 0) {
            // Success: debug print, re-enable interrupts and return to user code
            cprintf("dabort_handler: mapped va=0x%x for pid=%d, returning to user pc=0x%x\n",
                    va, proc->pid, r->pc);
            sti();
            return;
        }
    ...
    // If allocation failed or invalid address, print info and kill/process or panic
    cprintf("data abort: instruction 0x%x, fault addr 0x%x, reason 0x%x\n",
            r->pc, fault_addr, dfs);
    dump_trapframe(r);

    if (proc) {
        // If it's a user process and allocation failed, mark killed so it will exit
        proc->killed = 1;
        sti();
        return;
    }
